...
1.  **Simplicity First**: The format must be implementable in a small amount of
    C code (~500-1000 LOC) using only standard POSIX syscalls and a minimal
    MessagePack library.
2.  **Zero-Dependency Core**: Rely on the OS (filesystem atomicity, page cache,
...
*   **Mobile Optimized**: Coalesce updates every 2 seconds or 20 messages.
    ...
    forensic recovery extremely small.
...
*   **Push Back:** Implementing a robust, disk-backed B-Tree in C is a massive
    undertaking.
...
*   **Decision:** Kept both. **Fanout** reduces the range for "Found" lookups,
    while **Bloom Filters** (typically 2KB) provide extremely space-efficient
    rejection for "Not Found" lookups across multiple packs.
...
*   **Push Back:** Deleting the "future" (highest rank) makes the system
    ...
    low-rank junk. Furthermore, it requires maintaining a complex index for
    undecryptable data.
...
*   **Push Back:** Every `rename()` involves directory entry updates and inode
    allocation/deletion. In a high-throughput chat, this creates massive I/O
...
*   **Push Back:** Without a length prefix, the implementation cannot jump
    ...
    corrupted byte would make the entire remainder of the journal unreadable.
