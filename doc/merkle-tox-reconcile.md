...
linearly with the number of concurrent branches (heads). To support massive
group chats and efficient background synchronization of large histories, we
introduce **Invertible Bloom Lookup Tables (IBLT)** via the `tox-reconcile`
library.
...
## 1. Core Algorithm: Invertible Bloom Lookup Tables

An IBLT is a probabilistic data structure that allows for the recovery of
elements in a set difference.
...
2.  **Multicast Gossip**: A node multicasts a small sketch to a Tox Group. All
    ...
    packet to identify missing data across an entire 1,000-node swarm.
3.  **IBLT-Fountain Hybrid**: For extremely large sets, IBLT identifies *which*
    hashes are missing, and Fountain Coding (Erasure Coding) is used to
    broadcast the *content* of those nodes to ensure all peers converge
    simultaneously without redundant data transmissions.
