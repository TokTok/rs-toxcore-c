...
fn setup_model(msg_count: usize) -> Model {
        ...
        for i in 0..msg_count {
            conv.messages.push(Message {
                ...
                content: MessageContent::Text(format!(
                    "Message content with enough length to wrap significantly when resized {}",
                    i
                )),
...
fn bench_resize_layout(c: &mut Criterion) {
    let mut group = c.benchmark_group("resize_layout");
    group.sample_size(10);

    let message_counts = [1_000, 5_000, 10_000];

    for &count in &message_counts {
        group.bench_with_input(BenchmarkId::from_parameter(count), &count, |b, &c| {
            let mut model = setup_model(c);
            ...
            b.iter(|| {
                let width = if toggle { 100 } else { 120 };
                toggle = !toggle;

                // Resize the backend
                terminal.backend_mut().resize(width, 50);

                // Draw triggers re-layout because width changed
                terminal
                    .draw(|f| {
...
criterion_group!(benches, bench_resize_layout);
