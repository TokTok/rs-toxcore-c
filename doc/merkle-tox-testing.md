...
A critical feature of the benchmark suite is the ability to bridge simulated
...
enabling robust testing and predictable UI state transitions.
...
-   **Virtual Clock**: The `TimeProvider` from `merkle-tox-core` will be used to
    allow the TUI to "Pause" the entire swarm or "Step" it forward
    frame-by-frame.
...
The core benchmarking suite is **fully implemented** and available in
