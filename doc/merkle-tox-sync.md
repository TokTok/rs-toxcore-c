...
The synchronization protocol is responsible for ensuring that two peers
eventually hold the same set of DAG nodes for a conversation.
...
-   **Anchor Ad**: Members MUST also include the hash of their **Earliest Known
    ...
    acts as an **Explicit Vouch** for the entire chain of nodes between that
    anchor and the current heads, allowing joiners to bridge gaps larger than 16
    hops using data from blind relays.
...
traditional sense; they simply create multiple Heads.
